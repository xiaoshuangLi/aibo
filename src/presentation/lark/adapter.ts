 ...
 * Detection priority: explicit provider override → model name prefix.
 */
function isClaudeModel(): boolean {
  const { name: modelName, provider: explicitProvider } = config.model;
  if (explicitProvider) return explicitProvider === 'anthropic';
    ...
    try {
      ...
      if (messageType === 'image') {
        try {
          ...
          let url: string;
          if (isClaudeModel()) {
            // Claude 原生支持 base64 data URI，直接传递以避免额外上传开销
            const mimeType = detectImageMimeType(imageBuffer);
            url = `data:${mimeType};base64,${base64}`;
          } else {
            // 其他模型需要可公开访问的 URL，上传图片到素材库获取临时下载地址
            url = await this.chatService.uploadImage(base64);
          }
          // 构造多模态内容数组，将图片传给模型
