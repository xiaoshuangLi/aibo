 ...
 * - URL编码关键词以确保安全
 * - 构建Bing搜索URL
 * - 调用getTextFromUrl提取搜索结果页面内容
 * - 返回格式化的搜索结果文本
 * 
 * 行为分支：
 * 1. 正常搜索：成功构建URL并返回搜索结果内容
 * 2. 关键词处理：自动去除首尾空格并进行URL编码
 * 3. 错误处理：如果getTextFromUrl失败，错误会向上抛出
 * 
 * @param keyword - 要搜索的关键词字符串
 * @returns Promise<string> - Bing搜索结果页面的格式化文本内容Promise
 * 
 * @example
 * ```typescript
 * // 执行Bing搜索
 * const results = await bingSearch("JavaScript tutorial");
 * ```
 * 
 * @note
 * - 使用Bing搜索引擎
 * - 返回的内容包含搜索结果、相关链接等信息
 * - 依赖getTextFromUrl函数的完整功能
 */
