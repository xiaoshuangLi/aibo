 ...
 * 根据可用工具列表生成完整的系统提示词。
 *
 * 检测工具列表中是否存在各 AI 编程 CLI 工具，并将对应的路由提示词追加到基础系统提示词后面。
 *
 * @param tools - 工具对象数组，每个对象需包含 name 属性
 * @returns 完整的系统提示词字符串
 */
export function buildSystemPrompt(tools: Array<{ name: string }>): string {
  ...
  return SYSTEM_PROMPT + buildCodingAgentHint(hasClaudeTool, hasCursorTool, hasGeminiTool, hasCodexTool);
}
