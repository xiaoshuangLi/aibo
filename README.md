...
- 🔗 **MCP 工具集成**: 通过 `mcps/` 目录对接本机 Claude Code、Cursor 等编程 CLI 工具，或接入 GitHub、Slack 等外部服务
...
AIBO 通过 MCP 协议对接用户**本机已安装**的编程 CLI 工具，让这些工具作为 AIBO 的子任务执行器：

- 🤖 **Claude Code**：将编码子任务委派给本机的 Claude Code CLI 执行
- 🖱️ **Cursor**：通过 Cursor MCP Server 实现代码编辑与生成
- 🐙 **GitHub / Slack 等**：通过标准 MCP 配置接入第三方服务 API
...
| [docs/mcp.md](docs/mcp.md) | MCP 工具集成说明（连接本机 Claude Code、Cursor 等） |
