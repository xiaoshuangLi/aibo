...
- 🔗 **ACP 协议支持**: 通过统一的 ACP (Agent Client Protocol) 协议调用 13+ 种 AI 编码代理，支持持久化会话、命名并行会话、会话暂停/恢复
...
### 🔗 ACP 协议支持（Agent Client Protocol）

AIBO 现在支持通过 **ACP (Agent Client Protocol)** 协议统一调用 13+ 种 AI 编码代理，提供更灵活的会话管理和交互模式。
...
| 模式 | 说明 |
|------|------|
| `prompt` | 持久化会话模式（默认），支持多轮对话 |
| `exec` | 单次执行，不保存会话状态 |
| `sessions_new` | 创建新会话 |
| `sessions_list` | 列出所有会话 |
| `sessions_close` | 关闭指定会话 |
| `cancel` | 取消进行中的提示 |

**高级功能**:
- **持久化会话**: 支持多轮对话，保持上下文连续性
- **命名并行会话**: 通过 `session_name` 参数在同一项目中并行运行多个工作流
- **会话暂停/恢复**: 支持临时暂停会话并在需要时恢复
- **直传模式**: 允许用户直接与 AI 代理进行对话（通过 Lark 或 Console 适配器）

**使用示例**:
```typescript
// 单次执行
{ "agent": "codex", "prompt": "总结这个仓库的目的", "mode": "exec" }

// 持久会话 - 多轮对话
{ "agent": "claude", "mode": "sessions_new" }
{ "agent": "claude", "prompt": "找出 flaky 测试" }
{ "agent": "claude", "prompt": "应用修复并运行测试" }
...
> 📖 详细 ACP 使用指南请参阅 **[skills/acpx/SKILL.md](skills/acpx/SKILL.md)**
